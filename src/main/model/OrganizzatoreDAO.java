...
public class OrganizzatoreDAO {
    ...
    public Organizzatore doRetrieveByEmailPassword(String email, String password){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_or, piva, indirizzo_or, eventi_organizzati, numero_telefono_or FROM organizzatore WHERE email = ? AND passwordhash = SHA1(?)");
    ...
    public void doSave(Organizzatore organizzatore){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO organizzatore (piva, indirizzo_or, eventi_organizzati, numero_telefono_or, email, passwordhash) VALUES (?, ?, ?, ?, ?, SHA1(?))",
    ...
    public void doUpdate(Organizzatore organizzatore){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "UPDATE organizzatore SET piva=?, indirizzo_or=?, eventi_organizzati=?, numero_telefono_or=?, email=? WHERE id_or=?");
            ps.setString(1, organizzatore.getPiva());
            ps.setString(2, organizzatore.getIndirizzo_or());
            ps.setString(3, organizzatore.getEventi_organizzati());
            ps.setString(4, organizzatore.getNumero_telefono_or());
            ps.setString(5, organizzatore.getEmail());
            ps.setInt(6, organizzatore.getId_or());
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
