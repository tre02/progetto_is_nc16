...
public class EventoDAO {
    ...
    public Evento doRetrieveById(int id){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_evento, posti_disponibili, data_evento, luogo, ora, tipo FROM evento WHERE id_evento=?");
            ps.setInt(1, id);
    ...
    public List<Evento> doRetrieveAll(){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_evento, posti_disponibili, data_evento, luogo, ora, tipo FROM evento");
            ResultSet rs = ps.executeQuery();
