    ...
    public Amministratore doRetrieveByEmailPassword(String email, String password){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_am, orario_inizio_sessione, orario_fine_sessione, numero_telefono_am FROM amministratore WHERE email = ? AND password = SHA1(?)");
    ...
    public void doSave(Amministratore amministratore){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO amministratore (orario_inizio_sessione, orario_fine_sessione, numero_telefono_am, email, password) VALUES (?, ?, ?, ?, SHA1(?))",
