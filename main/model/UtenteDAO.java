...
public class UtenteDAO {
    ...
    public Utente doRetrieveByEmailPassword(String email, String password){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_utente, nome, cognome, passwordhash, email, admin FROM utente WHERE email = ? AND passwordhash =SHA1(?)");
    ...
    public void doSave(Utente utente){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO utente (nome,cognome, passwordhash, email, admin) VALUES(?,?,?,?,?,?,?,?,?,?)",
    ...
    public List<Utente> doRetriveAll(){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_utente, nome, cognome, passwordhash,email,admin FROM utente");
            ResultSet rs = ps.executeQuery();
    ...
    public Utente doRetrieveById_utente(int id_utente){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_utente, nome, cognome, passwordhash, email, admin FROM utente WHERE id_utente=?");
    ...
    public Utente doRetriveByEmail(String email){
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps =
                    con.prepareStatement("SELECT id_utente, nome, cognome, passwordhash, email, admin FROM utente WHERE email = ?");
            ps.setString(1, email);
            ResultSet rs = ps.executeQuery();
            if(rs.next()) {
                Utente user = new Utente();
